...
Let's view transfer example for contracting.


Contracting creation
~~~~~~~~~~~~~~~

At first let's create a contract:

.. include:: tutorial/create-contract.http
   :code:

`broker` is current contract's ``owner``.

Note that response's `access` section contains a ``transfer`` key which is used to change contract ownership. 

After contract's registration in CDB broker has to provide its customer with ``transfer`` key.
